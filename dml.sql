...
SELECT name, currentPrice AS price FROM Products;

SELECT name, atStore FROM Inventories;

SELECT Products.name as product, Inventories.name as location, ProductInventory.quantity
FROM Products INNER JOIN ProductsInventories ON Products.productID = ProductInventories.productID
INNER JOIN Inventories ON ProductsInventories.inventoryID = Inventories.inventoryID
ORDER BY product;
...
SUM(SELECT itemTotal FROM (SELECT (OrderProducts.quantity * Products.currentPrice) as itemTotal FROM OrderProducts INNER JOIN Products WHERE OrderProducts.productID = Products.productID) 
INNER JOIN ) 
...
= :certification_ID_selected_from-certificate_and_character_list
