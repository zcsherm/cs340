-- Changed all varchars to 255, easier,
-- Set name to unique, specify editions
...
VALUES (
    ...
    ((SELECT productID FROM Products WHERE name = 'Rhystic Study'), (SELECT inventoryID FROM Inventories WHERE name = 'Ordered'), 5)    
)
-- Insert Default values for Customers
INSERT INTO Customers (fname, lname, email, phone, points)
VALUES (
    ('Henry', 'Paul', 'hpaul@email.com', NULL),
    ('Jason', 'Isaacs', 'hizzy@me.net', '555-343-5288'),
    ('Monica', 'Isfran', 'spamfolder@yahoo.com', '5554876952'),
    ('Mokoto', 'Nagano', 'ninjawarrior@super.gove', NULL)
);
...
INSERT INTO orderProducts (orderID, itemID, quantity, inventoryID)
VALUES(
    (
    (SELECT orderID FROM (SELECT orderID FROM Orders INNER JOIN Customers ON Orders.customerID = Customers.customerID WHERE Customers.email = 'hizzy@me.net')),
    (SELECT productID FROM Products WHERE name = 'Fog'),
    13,
    (SELECT inventoryID FROM inventoryID WHERE NAME = 'Store Room'),  
    ),
    (
    (SELECT orderID FROM (SELECT orderID FROM Orders INNER JOIN Customers ON Orders.customerID = Customers.customerID WHERE Customers.email = 'hpaul@email.com')),
    (SELECT productID FROM Products WHERE name = 'Raging Goblin'),
    1,
    (SELECT inventoryID FROM inventoryID WHERE NAME = 'Display'),  
    ),
    (
    (SELECT orderID FROM (SELECT orderID FROM Orders INNER JOIN Customers ON Orders.customerID = Customers.customerID WHERE Customers.email = 'ninjawarrior@super.gove')),
